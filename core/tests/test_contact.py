...
def test_contact_valid_post_creates_lead_and_returns_success(client):
    ...
    assert lead.venue_name == "Momo Ghar Café"
...
def test_contact_missing_phone_is_rejected(client):
    resp = client.post(
        "/en/contact",
        {"name": "Suman Thapa", "venue_name": "Momo Ghar Café", "phone": ""},
        HTTP_HX_REQUEST="true",
    )
    assert resp.status_code == 200
    ...
    assert "Suman Thapa" in body  # submitted values preserved
...
def test_contact_non_htmx_post_renders_full_page(client):
    resp = client.post("/en/contact", {
        "name": "Suman", "venue_name": "Momo Ghar", "phone": "+977 98",
