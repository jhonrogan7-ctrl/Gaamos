...
Until phase 2 wires a 1024-d provider in, `PROVIDER` is None and this returns
None: the matcher's vector layer is simply off and every other layer works
without it (spec D6).
"""

DIMENSIONS = 1024

# Phase 2 assigns `embed_nv.embed` here. Tests inject via `embedder=`.
PROVIDER = None


def embed_text(text, *, embedder=None):
    """-> a DIMENSIONS-wide vector, or None when no catalog embedder is configured."""
    provider = embedder or PROVIDER
    if provider is None:
        return None
